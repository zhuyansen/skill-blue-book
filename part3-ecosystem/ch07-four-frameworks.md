...
>
> 2026 年初，4 个 Skill 元框架已经成形：anthropic/skills（标准工具箱）、obra/superpowers（强制方法论）、garrytan/gstack（虚拟工程团队）、kentcdodds/compound-engineering（知识飞轮）。这 4 条路线承载了 4 种不同的「Skill 是什么」的世界观。
>
> 本章用同一个测试任务——**「review 一段 React 组件代码」**——跑过 4 个框架，比较启动时间、上下文消耗、结果质量、定制成本。这是业内第一次有人做横向跑分。
...
这 4 个框架对「Skill 是什么」的答案完全不同。下面分别拆。
...
| 场景 | 推荐框架 | 备选 |
|------|------|------|
| 第一次玩 Skill，想看看是什么 | anthropic/skills | — |
| 生产环境代码，要求稳定 | superpowers | anthropic/skills（保底） |
| 独立开发者从 idea 到 launch | gstack | superpowers（关键代码段） |
| 学习 / 研究 Skill 本身的演化 | compound-engineering | superpowers |
| 团队 onboarding 新人 | anthropic/skills + gstack | — |
| 写一本书 / 做一个长期内容项目 | compound-engineering | — |
| 一次性 prototype / vibe coding | anthropic/skills 或不装 | — |

**注意：4 个框架不是「选一个就忠诚」**——很多重度用户**同时装** anthropic/skills（基础工具）+ superpowers（关键代码段）。Hub 数据里有 32% 的 Top 100 Skill 用户**装着至少两个框架**。
...
> **4 个 Skill 元框架对「Skill 是什么」给出了 4 个完全不同的答案：anthropic/skills 说它是工具，superpowers 说它是流程，gstack 说它是角色，compound-engineering 说它是有机体。这 4 种世界观最终会融合成一个还是分裂成 4 派，是 2026-2027 年生态最大的开放问题。**
